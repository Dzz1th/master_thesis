...
INFLATION_LEVEL_RANKING_PROMPT = """
    You are an expert in macroeconomics and central bank policy. You are analyzing Federal Reserve statements about inflation levels across two periods. 
...
You are an expert in macroeconomics and central bank policy. Your task is to analyze Federal Reserve statements on balance sheet policy across two periods and determine whether the guidance in Set B (later statement) suggests a shift toward a more accommodative (easing) stance, a more restrictive (tightening) stance, or remains unchanged compared to Set A (earlier statement).
...
You are an expert in macroeconomics and central bank policy. Your task is to analyze Federal Reserve statements on interest rate guidance across two periods and determine whether the guidance in Set B (later statement) is more explicit, more implicit (data-dependent), or remains at the same level of clarity compared to Set A (earlier statement).
