...
class LinearRankNet:
    ...
    def apply_pca(self, features: np.ndarray, 
        ...
        if pca_components > 0:
            self.feature_dim = pca_components
            if mode == 'train':
    ...
    def _compute_gradients(self, 
        ...
        for k in range(batch_size):
            ...
            if r == 1:  # First doc ranked higher (yi > yj)
                ...
                factor = -1 * sigma * exp_term / (1 + exp_term)  # Derivative of loss w.r.t difference
    ...
    def _evaluate_metrics(self, 
        ...
        Returns:
            ...
            - f1_scores: Dictionary with class-wise F1 scores
        ...
        # Lists to store true and predicted labels for F1 computation
        y_true = []
        y_pred = []
        ...
        return {
            ...
            'accuracy': accuracy,
            'confusion_matrix': cm,
            **f1_scores,  # Include individual class F1 scores
            'f1_macro': f1_macro,
            'f1_micro': f1_micro
        }
    ...
    def predict(self, 
        ...
        if self.model_params['pca_components'] > 0:
            features = self.apply_pca(features, self.model_params['pca_components'], 'predict')
            self.feature_dim = self.model_params['pca_components']
    ...
    def hyperparameter_search(self, 
                        ...
                        target_metrics: t.Literal['accuracy', 'f1_macro', 'f1_micro'] = 'accuracy',
        ...
        """
        self.logger.info(f"Starting hyperparameter search")
        ...
        for values in itertools.product(*param_values):
            ...
            self.logger.info(f"Training loss: {history['train_loss'][-1]:.4f}, Training accuracy: {history['train_accuracy'][-1]:.4f}")
            self.logger.info(f"Validation loss: {val_loss:.4f}, accuracy: {val_metrics['accuracy']:.4f}, Validation Confusion Matrix:\n{val_metrics['confusion_matrix']} \n")
            ...
            if val_metrics[target_metrics] > best_metrics[target_metrics]:
                best_metrics = val_metrics
                best_params = current_params.copy()
                self.logger.info(f"New best parameters found with validation {target_metrics}: {best_metrics[target_metrics]:.4f}")
        ...
        self.logger.info(f"Best training loss: {best_train_loss:.4f}, Best training accuracy: {best_train_accuracy:.4f}, Params: {best_train_params}")
        ...
        if best_params:
            ...
            train_data_copy['features'] = self.apply_pca(train_data_copy['features'], 
                                                self.model_params['pca_components'], 
                                                'train')
            val_data_copy['features'] = self.apply_pca(val_data_copy['features'], 
                                                self.model_params['pca_components'], 
                                                'val')
            ...
            self.logger.info(f"Best validation {target_metrics}: {best_metrics[target_metrics]:.4f}")

            # Retrain model with best parameters
            history = self.fit(train_data_copy, val_data_copy)
            train_metrics = self._evaluate_metrics(
                train_data_copy['features'],
                train_data_copy['first_idxs'],
                train_data_copy['second_idxs'],
                train_data_copy['rankings']
            )

            self.logger.info(f"Best training accuracy: {train_metrics['accuracy']:.4f}")
            self.logger.info(f"Best training confusion matrix:\n{train_metrics['confusion_matrix']}")
        else:
